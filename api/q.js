...
const CF_TOKEN   = () => process.env.CF_API_TOKEN || process.env.BBSD_CF_TOKEN || process.env.AI_Gateway_XP;
...
async function kvSet(key, value) {
  ...
  if (!r.ok) {
    ...
    console.error(`KV write failed ${r.status}: ${body}`);
    return false;
  }
  return true;
}
...
function makeSlug(name) {
  return name.toLowerCase().replace(/[^a-z0-9\s-]/g, '').trim().split(/\s+/)[0].slice(0, 20);
}
...
export default async function handler(req) {
  ...
  if (req.method === 'POST') {
    ...
    if (!destUrl) return new Response(
    ...
    const ok = await kvSet(finalSlug, destUrl);
    if (!ok) {
      // KV write failed — return the full URL instead so quote still works
      return new Response(
        JSON.stringify({ error: 'KV write failed — check CF_API_TOKEN permissions', shortUrl: destUrl, slug: finalSlug }),
        { status: 500, headers: { ...cors, 'Content-Type': 'application/json' } }
      );
    }

    return new Response(
      JSON.stringify({ slug: finalSlug, shortUrl: SHORT_BASE + finalSlug, url: destUrl }),
